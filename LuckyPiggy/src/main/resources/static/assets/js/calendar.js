...
let days = calist.map(item => item.paidAt)
let mbAmount = calist.map(item => item.mbAmount !== null ? parseFloat(item.mbAmount.replace(/,/g, '')) : 0);
let mbIc = calist.map(item => item.mbIc !== null ? parseFloat(item.mbIc.replace(/,/g, '')) : 0);
...
darray.forEach((d, index) => {
	...
	if(expenseData[d]) {
		expenseData[d].mbAmount += mbAmount[index];
		expenseData[d].mbIc += mbIc[index];
	}
	else {
	expenseData[d] = {
		mbAmount : mbAmount[index],
		mbIc : mbIc[index]
		};
	};
});
...
const renderCalendar = () => {
   ...
   for (let i = 1; i <= lastDateOfMonth; i++) {
      ...
      let income = expense.mbIc > 0 ? `<div class="income">${expense.mbIc}</div>` : '';
      let expenseText = expense.mbAmount > 0 || expense.Amount > 0 ? `<div class="expense">${expense.mbAmount}</div>` : '';
