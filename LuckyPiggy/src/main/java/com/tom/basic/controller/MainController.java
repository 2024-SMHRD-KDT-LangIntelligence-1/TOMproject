...
public class MainController {
	...
	public String daily(HttpServletRequest request, Model model,
		...
		int totalIncome = incomeList.stream().mapToInt(entry -> parseAmount(entry.getMbIc())) // String을 int로 변환
				.sum();
		...
		int totalExpense = expenseList.stream().mapToInt(entry -> parseAmount(entry.getMbAmount())) // String을 int로 변환
				.sum();
...
//		model.addAttribute("moneybook_list", moneybook_list);
	...
	public String graph(Model model, HttpServletRequest request) {
		...
		List<lineVO> linetotal = graphRepo.getlinetotal(month, userid);
		System.out.println("먼스는" + month + linetotal);
